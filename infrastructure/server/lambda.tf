...
resource "aws_iam_role" "get_score_lambda_role" {
  name = "get-score-lambda-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "get_score_lambda_policy_attachment" {
  role       = aws_iam_role.get_score_lambda_role.name
  policy_arn = aws_iam_policy.get_score_lambda_policy.arn
}

data "archive_file" "get_score_lambda" {
  type        = "zip"
  source_dir  = "${local.lambda_source_path}/get-score"
  output_path = "temp/artifacts/get-score-lambda.zip"
}

resource "aws_lambda_function" "get_score_lambda" {
  function_name    = "get-score-lambda"
  filename         = data.archive_file.get_score_lambda.output_path
  source_code_hash = data.archive_file.get_score_lambda.output_base64sha256
  handler          = "main.lambda_handler"
  runtime          = "python3.8"
  role             = aws_iam_role.get_score_lambda_role.arn
}

resource "aws_lambda_permission" "get_score_lambda_permission" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.get_score_lambda.arn
  principal     = "apigateway.amazonaws.com"
}

resource "aws_cloudwatch_log_group" "get_score_lambda" {
  name              = "/aws/lambda/${aws_lambda_function.get_score_lambda.function_name}"
  retention_in_days = 14
}

data "aws_iam_policy_document" "get_score_lambda" {
  statement {
    actions = [
      "logs:CreateLogStream",
      "logs:PutLogEvents"
    ...
    resources = [
      aws_cloudwatch_log_group.get_score_lambda.arn,
      "${aws_cloudwatch_log_group.get_score_lambda.arn}:*"
    ]
  }
}

resource "aws_iam_policy" "get_score_lambda_logging" {
  name   = "get-score-lambda-logging"
  policy = data.aws_iam_policy_document.get_score_lambda.json
}

resource "aws_iam_role_policy_attachment" "get_score_lambda_logging" {
  role       = aws_iam_role.get_score_lambda_role.name
  policy_arn = aws_iam_policy.get_score_lambda_logging.arn
}
